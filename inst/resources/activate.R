#### Customization of 'activate.R' to make it work on KPT-VDI ####

lib_root <- file.path(
  "C:/renv_libs/",
  getwd() |> digest::digest(algo = "sha1")
)
dir.create(lib_root, showWarnings = FALSE)

Sys.setenv(
  RENV_PATHS_LIBRARY_ROOT = lib_root,
  RENV_PATHS_LIBRARY_STAGING = file.path(lib_root, "staging"),
  RENV_PATHS_LIBRARY = file.path(lib_root, "lib")
)
...
local({
    ...
    on.exit(
      {
        ...
        writeLines(
          sprintf("- renv took %s to run the autoloader.", format(elapsed))
        )
  ...
  enabled <- local({
    ...
    if (!is.na(rlibs) && !is.na(rcmd)) return(FALSE)
    ...
    for (envvar in envvars) {
      ...
      if (!is.na(envval)) return(tolower(envval) %in% c("true", "t", "1"))
    }
  ...
  ansify <- function(text) {
    if (renv_ansify_enabled()) renv_ansify_enhanced(text) else
      renv_ansify_default(text)
  }

  renv_ansify_enabled <- function() {
    override <- Sys.getenv("RENV_ANSIFY_ENABLED", unset = NA)
    if (!is.na(override)) return(as.logical(override))

    pane <- Sys.getenv("RSTUDIO_CHILD_PROCESS_PANE", unset = NA)
    if (identical(pane, "build")) return(FALSE)

    testthat <- Sys.getenv("TESTTHAT", unset = "false")
    if (tolower(testthat) %in% "true") return(FALSE)

    iderun <- Sys.getenv("R_CLI_HAS_HYPERLINK_IDE_RUN", unset = "false")
    if (tolower(iderun) %in% "false") return(FALSE)
  ...
  renv_ansify_init <- function() {
    ...
    if (renv_ansify_enabled())
      assign("ansify", renv_ansify_enhanced, envir = envir) else
  ...
  header <- function(
    ...
    if (n <= 0) return(paste(prefix, label))
  ...
  renv_bootstrap_repos <- function() {
    ...
    if (!is.na(repos)) {
      ...
      if (identical(rspm, repos)) repos <- c(RSPM = rspm, CRAN = cran)
    ...
    if (!inherits(repos, "error") && length(repos)) return(repos)
  ...
  renv_bootstrap_download <- function(version) {
    ...
    for (method in methods) {
      ...
      if (is.character(path) && file.exists(path)) return(path)
    }
  ...
  renv_bootstrap_download_impl <- function(url, destfile) {
    ...
    if ("headers" %in% names(formals(utils::download.file))) {
      ...
      if (length(headers) && is.character(headers)) args$headers <- headers
    }
  ...
  renv_bootstrap_download_custom_headers <- function(url) {
    ...
    if (is.null(headers)) return(character())

    if (!is.function(headers))
      stopf("'renv.download.headers' is not a function")

    headers <- headers(url)
    if (length(headers) == 0L) return(character())
  ...
  renv_bootstrap_download_cran_latest <- function(version) {
    ...
    ext <- if (identical(type, "source")) ".tar.gz" else if (
      Sys.info()[["sysname"]] == "Windows"
    )
      ".zip" else ".tgz"
    ...
    if (inherits(status, "condition")) return(FALSE)
  ...
  renv_bootstrap_download_cran_archive <- function(version) {
    ...
    for (url in urls) {
      ...
      if (identical(status, 0L)) return(destfile)
    }
  ...
  renv_bootstrap_github_token <- function() {
    for (envvar in c("GITHUB_TOKEN", "GITHUB_PAT", "GH_TOKEN")) {
      envval <- Sys.getenv(envvar, unset = NA)
      if (!is.na(envval)) return(envval)
    }
  }

  renv_bootstrap_download_github <- function(version) {
    enabled <- Sys.getenv("RENV_BOOTSTRAP_FROM_GITHUB", unset = "TRUE")
    if (!identical(enabled, "TRUE")) return(FALSE)
  ...
  renv_bootstrap_install <- function(version, tarball, library) {
    ...
    if (is.null(status) || identical(status, 0L)) return(status)
  ...
  renv_bootstrap_platform_prefix <- function() {
    ...
    if (devel) prefix <- paste(prefix, R.version[["svn rev"]], sep = "-r")
    ...
    if (!is.na(prefix) && nzchar(prefix)) components <- c(prefix, components)
  ...
  renv_bootstrap_platform_prefix_impl <- function() {
    ...
    if (is.na(auto) && getRversion() >= "4.4.0") auto <- "TRUE"
  ...
  renv_bootstrap_platform_os <- function() {
    ...
    if (sysname == "Windows") return("windows") else if (sysname == "Darwin")
      return("macos")
  ...
  renv_bootstrap_platform_os_via_redhat_release <- function() {
    ...
    # infer id
    id <- if (grepl("centos", contents, ignore.case = TRUE)) "centos" else if (
      grepl("redhat", contents, ignore.case = TRUE)
    )
      "redhat" else "unknown"
  ...
  renv_bootstrap_library_root_name <- function(project) {
    ...
    if (asis) return(basename(project))
  ...
  renv_bootstrap_library_root <- function(project) {
    ...
    if (!is.na(path)) return(paste(c(path, prefix), collapse = "/"))
  ...
  renv_bootstrap_validate_version <- function(version, description = NULL) {
    ...
    valid <- if (!is.null(sha))
      renv_bootstrap_validate_version_dev(sha, description) else
    ...
    remote <- if (dev)
      paste("rstudio/renv", description[["RemoteSha"]], sep = "@") else
  ...
  renv_bootstrap_validate_version_dev <- function(version, description) {
    ...
    if (!is.character(expected)) return(FALSE)
  ...
  renv_bootstrap_load <- function(project, libpath, version) {
    ...
    for (hook in hooks) if (is.function(hook)) tryCatch(hook(), error = warnify)
  ...
  renv_bootstrap_profile_load <- function(project) {
    ...
    if (!is.na(profile) && nzchar(profile)) return(profile)
    ...
    if (length(contents) == 0L) return(NULL)

    # set RENV_PROFILE
    profile <- contents[[1L]]
    if (!profile %in% c("", "default")) Sys.setenv(RENV_PROFILE = profile)
  ...
  renv_bootstrap_profile_prefix <- function() {
    ...
    if (!is.null(profile)) return(file.path("profiles", profile, "renv"))
  }
  ...
  renv_bootstrap_profile_set <- function(profile) {
    ...
    if (is.null(profile)) Sys.unsetenv("RENV_PROFILE") else
      Sys.setenv(RENV_PROFILE = profile)
  }

  renv_bootstrap_profile_normalize <- function(profile) {
    if (is.null(profile) || profile %in% c("", "default")) return(NULL)
  ...
  renv_bootstrap_project_type <- function(path) {
    ...
    if (!file.exists(descpath)) return("unknown")
    ...
    if (inherits(desc, "error")) return("unknown")

    type <- desc$Type
    if (!is.null(type)) return(tolower(type))

    package <- desc$Package
    if (!is.null(package)) return("package")
  ...
  renv_bootstrap_user_dir_impl <- function() {
    ...
    if (is.function(tools$R_user_dir)) return(tools$R_user_dir("renv", "cache"))

    # try using our own backfill for older versions of R
    envvars <- c("R_USER_CACHE_DIR", "XDG_CACHE_HOME")
    for (envvar in envvars) {
      root <- Sys.getenv(envvar, unset = NA)
      if (!is.na(root)) return(file.path(root, "R/renv"))
    }

    # use platform-specific default fallbacks
    if (Sys.info()[["sysname"]] == "Windows")
      file.path(Sys.getenv("LOCALAPPDATA"), "R/cache/R/renv") else if (
  ...
  renv_json_read <- function(file = NULL, text = NULL) {
    ...
    if ("jsonlite" %in% loadedNamespaces()) {
      ...
      if (!inherits(json, "error")) return(json)
    ...
    # report an error
    if (!is.null(jlerr)) stop(jlerr) else stop(json)
  }
